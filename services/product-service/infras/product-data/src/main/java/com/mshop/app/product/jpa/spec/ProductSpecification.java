...
public class ProductSpecification {
    ...
    public static Specification<ProductEntity> findByCategoryId(String categoryPath) {
        return (root, query, builder) -> {
            if (query == null) {
            ...
            subquery.select(productCategoryRoot.get(productIdField))
                    .where(
                            builder.equal(
                                    productCategoryRoot.get(categoryIdField),
                                    categoryRoot.get(CategoryField.ID.getField())
                            ),
                            builder.or(
                                    builder.equal(
                                            categoryRoot.get(CategoryField.PATH.getField()),
                                            categoryPath
                                    ),
                                    builder.like(
                                            categoryRoot.get(CategoryField.PATH.getField()),
